...
def search_results(request):
    if 'searchItem' in request.GET and request.GET["searchItem"]:
        ...
        return render(request, 'neighbours/search.html', context)

    else:
        messages.success(request, f"You haven't searched for any term")

        return render(request, 'neighbours/search.html',{"message":message})

def singlebsnview(request,post_id):
    post = get_object_or_404(Business,pk=post_id)
    return render(request, 'neighbours/business_detail.html',locals())



# class PostListView(ListView):
#     context_object_name = 'post_list'
#     template_name= 'neighbours/post_list.html' # <app>/<model>_<view_type>.html
...
def post_listview(request):
    ...
    return render(request,'neighbours/post_list.html', locals())
...
def business_listview(request):
    ...
    return render(request,'neighbours/business_list.html', locals())
def change_user_role(request,user_id):
    ...
    return (request,'neighbours/post_list.html')
# def business_listview(request):
#     location = get_object_or_404(Neighbourhood, pk=request.user.profile.community.id)
#     try:
...
class PostCreateView(CreateView):
    ...
    template_name = 'neighbours/create_post.html'
...
class BusinessCreateView(CreateView):
    ...
    template_name = 'neighbours/create_business.html'
